...
object TimeUsage extends TimeUsageInterface:
  ...
  def row(line: List[String]): Row = {
    // First element as String, the rest as Double
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val (primaryNeeds, work, other) = columnNames.foldLeft((List.empty[Column], List.empty[Column], List.empty[Column])) {
      case ((primary, work, other), colName) =>
        if (primaryNeedsPrefixes.exists(colName.startsWith)) (primary :+ col(colName), work, other)
        else if (workPrefixes.exists(colName.startsWith)) (primary, work :+ col(colName), other)
        else if (otherPrefixes.exists(colName.startsWith)) (primary, work, other :+ col(colName))
        else (primary, work, other)
    }

    (primaryNeeds, work, other.filterNot(col => primaryNeeds.contains(col) || work.contains(col)))
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection = when($"telfs" < 3, "working").otherwise("not working").as("working")
    val sexProjection = when($"tesex" === 1, "male").otherwise("female").as("sex")
    val ageProjection = when($"teage".between(15, 22), "young").when($"teage".between(23, 55), "active").otherwise("elder").as("age")

    val primaryNeedsProjection = primaryNeedsColumns.reduce(_ + _).divide(60).as("primaryNeeds")
    val workProjection = workColumns.reduce(_ + _).divide(60).as("work")
    val otherProjection = otherColumns.reduce(_ + _).divide(60).as("other")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    summed.groupByKey(row => (row.working, row.sex, row.age))
      ...
      .map {
        case ((working, sex, age), primaryNeedsAvg, workAvg, otherAvg) =>
          TimeUsageRow(
            working, 
            sex, 
            age, 
            Math.round(primaryNeedsAvg * 10) / 10.0, 
            Math.round(workAvg * 10) / 10.0, 
            Math.round(otherAvg * 10) / 10.0
          )
      }
  }
